class ClubManagementController < ApplicationController
  before_action :login_only, :init, except: %i[get_qrcode check_qrcode]
  before_action :manager_only, except: %i[get_qrcode bind_qrcode check_qrcode verify_qrcode]
  ...
  def club_update
    (_icon_image = Image.find_by(token: params[:icon_token])) && (@club.icon_image_id = _icon_image.id)
    @club.english_name = params[:english_name]
  ...
  def activity_create
    ...
    (_post_horizontal_image = Image.find_by(token: params[:post_horizontal_image_token])) &&
      (activity.post_horizontal_image_id = _post_horizontal_image.id)
    (_post_vertical_image = Image.find_by(token: params[:post_vertical_image_token])) &&
  ...
  def activity_update
    ...
    if _host_clubs
      _host_clubs.each do |club_name|
        ...
        next if @activity.clubs.include? club
        club_to_activity = ClubToActivity.new
        club_to_activity.club_id = club.id
        club_to_activity.activity_id = @activity.id
        club_to_activity.save!
  ...
  def article_create
    ...
    (_cover_image = Image.find_by(token: params[:cover_image_token])) && (article.cover_image_id = _cover_image.id)
